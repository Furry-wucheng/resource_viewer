...
| 阶段 | 状态 | 说明 |
|------|------|------|
| 01 项目脚手架与基础 UI | ✅ 已完成 | 主题/DI/路由/响应式导航/3 Tab 壳 |
| 02 数据层与基础设施 | ✅ 已完成 | drift 建表/freezed/Repository/错误处理 |
| 03 本地源链路 | ✅ 已完成 | LocalFileSource→文件浏览器→查看器→视频→首页网格→缩略图预览 |
| 04 标签系统与筛选 | ⬜ 待开始 | Tag CRUD→筛选栏交集筛选→批量打标签→标签管理页 |
| 05 SMB 网络源 | ⬜ 待开始 | SmbFileSource→凭据安全→可用性检测→跨源统一 |
| 06 组织模式与高级查看 | ⬜ 待开始 | 4 种策略→PDF→画廊→ResourcePicker→拆分资源 |
| 07 设置打磨与测试 | ⬜ 待开始 | 设置页→ZIP 阅读→性能优化→五层测试覆盖 |

### 最近更新

- **2026-06-21**：阶段 3 收尾 — 文件浏览器增强
  - 缩略图预览：网格/列表双视图均支持图片和视频文件实时预览
  - 预览压缩：图片预览统一压缩到 180×270（quality 85），在 Isolate 中执行，避免原图全尺寸字节驻留内存
  - 缩略图参数统一：`ThumbnailGenerator` 基类定义 `thumbWidth` / `thumbHeight` / `jpegQuality` 常量，所有生成器和预览共用
  - 文件夹标识：网格和列表视图中文件夹条目右下角显示小文件夹角标，便于区分文件夹和文件
  - 视图模式持久化：用户选择的网格/列表模式通过 SharedPreferences 保存，下次打开自动恢复
  - 网格文件名渐变：文件名浮在图片底部，半透明黑色渐变覆盖，视觉效果统一
